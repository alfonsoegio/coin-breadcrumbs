    ...
    for (let i = 0; i < this.keys.key.length; i++) {
      if (this.keys.key[i].address === this.last_address.address) {
        ...
        if (balance.data['confirmed_balance'] > 0) {
          ...
          let unbalance = await chainClient.getAddressBalance(this.keys.key[i].address, 0)
          if ((unbalance.data['unconfirmed_balance'] === null || unbalance.data['unconfirmed_balance'] < DEFAULT_FEE) &&
              unbalance.data['confirmed_balance'] < DEFAULT_FEE) {
            this.generateNewKey()
            this.last_address = this.keys.key[this.keys.key.length - 1]
